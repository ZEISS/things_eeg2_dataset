...
class StageReport:
    ...
    files_produced: List[str] = None

    # Fields for the downloading step of the process
    requested_subjects: Optional[List[int]] = None
    downloaded_subjects: Optional[List[int]] = None
    source_data_downloaded: Optional[bool] = None
    missing_files: Optional[List[str]] = None
    total_download_size: Optional[float] = None  # in GB
    
    # Fields for the preprocessing step
    sampling_frequency: Optional[int] = None  # Target sampling frequency in Hz
    original_sampling_frequency: Optional[int] = None  # Original sampling frequency
    processing_tasks: Optional[List[str]] = None  # List of processing tasks performed
    filter_type: Optional[str] = None  # Type of filter applied (e.g., "low-pass FIR")
    filter_frequencies: Optional[Dict[str, float]] = None  # Filter frequencies (l_freq, h_freq)
    epoch_time_window: Optional[Dict[str, float]] = None  # Epoching time window (tmin, tmax)
    baseline_correction: Optional[Dict[str, Any]] = None  # Baseline correction parameters
    num_channels: Optional[int] = None  # Number of EEG channels
    num_sessions_processed: Optional[int] = None  # Number of sessions processed
    
    # Fields for the embedding generation step
    model_name: Optional[str] = None  # Name of the embedding model
    batch_size: Optional[int] = None  # Batch size used for embedding generation
    embedding_dimension: Optional[Dict[str, Any]] = None  # Embedding dimensions (pooled, full)
    num_images_processed: Optional[Dict[str, int]] = None  # Number of images per partition
    precision: Optional[str] = None  # Model precision (e.g., "float16", "float32")
    embedding_variants: Optional[List[str]] = None  # Types of embeddings generated (pooled, full)
    
    # Fields for validation results
    validation_status: Optional[str] = None  # Overall validation status: "passed", "warning", "failed"
    validation_checks_passed: Optional[int] = None  # Number of validation checks passed
    validation_checks_failed: Optional[int] = None  # Number of validation checks failed
    validation_warnings: Optional[int] = None  # Number of validation warnings
    validation_issues: Optional[List[Dict[str, Any]]] = None  # List of validation issues
...
class PipelineReport:
    ...
    total_elapsed_time: Optional[float] = None  # Time in seconds
    ...
    n_subjects_processed: Optional[int] = None  # Number of subjects of the dataset
    
    # Stage reports
    stages: List[StageReport] = None
...
class ReportGenerator:
    ...
    def __init__(self, output_dir: Path, pipeline_version: str, dataset_name: str):
        """
    ...
    def generate_text_report(self) -> str:
        ...
        if self.report.execution_end:
            ...
            lines.append(f"Total Elapsed Time: {self._format_time(self.report.total_elapsed_time)}")
        lines.append("")
        ...
        lines.append("-" * 80)
        lines.append(f"Number of Subjects Processed: {self.report.n_subjects_processed or 'N/A'}")
        ...
        lines.append("-" * 80)
        executed = sum(1 for s in self.report.stages if s.status == StageStatus.EXECUTED)
        ...
        for i, stage in enumerate(self.report.stages, 1):
            ...
            if stage.elapsed_time is not None:
                lines.append(f"    Elapsed Time: {self._format_time(stage.elapsed_time)}")
            ...
            if stage.requested_subjects is not None:
                ...
                lines.append(f"      - Requested: {len(stage.requested_subjects or [])} subjects")
                lines.append(f"      - Downloaded: {len(stage.downloaded_subjects or [])} subjects")
                if stage.total_download_size is not None:
                    lines.append(f"      - Total Size: {stage.total_download_size:.2f} GB")
                if stage.missing_files:
                    lines.append(f"      - Missing Files: {len(stage.missing_files)} issues detected")
            
            # Preprocessing-specific statistics
            if stage.sampling_frequency is not None:
                lines.append("    Preprocessing Statistics:")
                lines.append(f"      - Sampling Frequency: {stage.original_sampling_frequency} Hz → {stage.sampling_frequency} Hz")
            ...
            if stage.model_name is not None:
                ...
                if stage.embedding_variants:
                    lines.append(f"      - Variants: {', '.join(stage.embedding_variants)}")
            ...
            if stage.validation_status is not None:
                ...
                lines.append(f"      - Status: {emoji} {stage.validation_status.upper()}")
                
                if stage.validation_checks_passed is not None:
                    lines.append(f"      - Checks Passed: {stage.validation_checks_passed}")
                if stage.validation_warnings is not None and stage.validation_warnings > 0:
                    lines.append(f"      - Warnings: {stage.validation_warnings}")
                if stage.validation_checks_failed is not None and stage.validation_checks_failed > 0:
                    lines.append(f"      - Checks Failed: {stage.validation_checks_failed}")
                ...
                if stage.validation_issues and len(stage.validation_issues) > 0:
                    ...
                    for issue in stage.validation_issues[:5]:  # Show first 5 issues
                        ...
                        lines.append(f"        [{severity.upper()}] {check_name}: {message}")
                    
                    if len(stage.validation_issues) > 5:
                        lines.append(f"        ... and {len(stage.validation_issues) - 5} more issues")
            
            if stage.files_produced:
                lines.append(f"    Files Produced: {len(stage.files_produced)}")
                for file in stage.files_produced[:10]:  # Show first 10 files only for avoiding overcluttering 
                    lines.append(f"      - {file}")
                if len(stage.files_produced) > 10:
    ...
    def generate_stage_text_report(self, stage: StageReport) -> str:
        ...
        if stage.requested_subjects is not None:
            ...
            lines.append(f"Successfully Downloaded: {len(stage.downloaded_subjects or [])}/{len(stage.requested_subjects or [])}")
            lines.append(f"Source Data Downloaded: {'Yes' if stage.source_data_downloaded else 'No'}")
        ...
        if stage.sampling_frequency is not None:
            ...
            if stage.original_sampling_frequency:
                lines.append(f"Original Sampling Frequency: {stage.original_sampling_frequency} Hz")
            ...
            if stage.num_sessions_processed:
                lines.append(f"Sessions Processed per Subject: {stage.num_sessions_processed}")
            ...
            if stage.filter_type or stage.filter_frequencies:
                ...
                if stage.filter_frequencies:
                    ...
                    lines.append(f"  Low Frequency: {l_freq} Hz" if l_freq != 'None' else f"  Low Frequency: None (high-pass not applied)")
                    lines.append(f"  High Frequency: {h_freq} Hz" if h_freq != 'None' else f"  High Frequency: None (low-pass not applied)")
            ...
            if stage.baseline_correction:
                ...
                lines.append(f"  Baseline Period: [{baseline_start}, {baseline_end}] seconds")
