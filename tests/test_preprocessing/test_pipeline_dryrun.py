...
def mock_project_dir(tmp_path: Path) -> Path:
    ...
    (tmp_path / "raw_data").mkdir() # Where tmp_path is a temporary path that is automatically cleaned up after the pipeline dry-run test
    ...
    for subject in [1, 2]:
        for session in range(1, 5):
            session_dir = tmp_path / "raw_data" / f"sub-{subject:02d}" / f"ses-{session:02d}"
...
class TestDryRunConfig:
    ...
    @pytest.mark.parametrize("dry_run_value", [True, False]) # Checks if the flag is correctly set for both True and False
    def test_dry_run_flag_initialization(self, mock_project_dir: Path, dry_run_value: bool):
...
class TestDryRunIntegration:
    ...
    def test_full_pipeline_executes_all_stages(
        self,
        mock_input,
        mock_torch,
        mock_validate_processed,
        mock_validate_raw,
        mock_build_embedder,
        ...
        config = PipelineConfig(
            project_dir=mock_project_dir,
            subjects=[1, 2],
            models=[EmbeddingModel.OPEN_CLIP_VIT_H_14], # Use of specific model for embedding generation (can be modified if required)
            dry_run=True,
            interactive=False,
        )
    ...
    def test_no_files_written_in_dry_run(self, mock_project_dir: Path):
        ...
        with patch("things_eeg2_dataset.processing.pipeline.Downloader"), \
             patch("things_eeg2_dataset.processing.pipeline.RawProcessor"), \
             patch("things_eeg2_dataset.processing.pipeline.build_embedder"), \
             patch("things_eeg2_dataset.processing.pipeline.validate_raw_data") as mock_validate, \
             patch("things_eeg2_dataset.processing.pipeline.validate_processed_data"), \
             patch("things_eeg2_dataset.processing.pipeline.torch"), \
             patch("builtins.input", return_value="n"):
            
            mock_validate.return_value = Mock(has_failures=False, has_warnings=False, issues=[])
